...
To ensure videos work in Chrome/Firefox/Safari/Edge, videos should be encoded with both H.264 MP4 and VP9 WebM. For example:

```bash
# use this command to generate the H.264 MP4 version of the video
ffmpeg -i assets/Chia_Launch.mp4 -c:v libvpx-vp9 -b:v 2M assets/Chia_Launch.webm # convert video (in any supported format) into VP9 WebM

# use this command to generate the VP9 WebM version of the video
ffmpeg -i assets/Chia_Launch.webm -c:v libx264 -crf 23 -maxrate 2M -bufsize 2M assets/Chia_Launch.mp4 # convert video (in any supported format) into H.264 MP4
```
...
Now that you have the VP9 WebM, the H.264 MP4, and the thumbnail JPG, use the following markup in your HTML to include the video (with our cross-browser video component):

```
{% include video.html path="/assets/Chia_Launch.webm" mimetype="video/webm" fallback_path="/assets/Chia_Launch.mp4" fallback_mimetype="video/mp4" thumbnail="/assets/Chia_Launch.jpg" %}
```
